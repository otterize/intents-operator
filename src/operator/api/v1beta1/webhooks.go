...
func (in *ClientIntents) ConvertTo(dstRaw conversion.Hub) error {
	...
	dst.Spec.Targets = make([]v2beta1.Target, len(in.Spec.Calls))
	for i, call := range in.Spec.Calls {
		if call.IsTargetInCluster() && call.Type != IntentTypeKafka {
			...
			kubernetesTarget := v2beta1.KubernetesTarget{Name: name, Kind: call.GetTargetServerKind()}
			...
			if call.Type == IntentTypeHTTP && len(call.HTTPResources) > 0 {
				httpTargets := make([]v2beta1.HTTPTarget, len(call.HTTPResources))
				for j, http := range call.HTTPResources {
					methods := lo.Map(http.Methods, func(method HTTPMethod, _ int) v2beta1.HTTPMethod { return v2beta1.HTTPMethod(method) })
					httpTargets[j] = v2beta1.HTTPTarget{Path: http.Path, Methods: methods}
				}
				kubernetesTarget.HTTP = httpTargets
			}
			dst.Spec.Targets[i] = v2beta1.Target{Kubernetes: &kubernetesTarget}
			continue
		}
		if call.Type == IntentTypeKafka && len(call.Topics) > 0 {
			topics := lo.Map(call.Topics, func(topic KafkaTopic, _ int) v2beta1.KafkaTopic {
				return v2beta1.KafkaTopic{Name: topic.Name, Operations: lo.Map(topic.Operations, func(operation KafkaOperation, _ int) v2beta1.KafkaOperation {
					return v2beta1.KafkaOperation(operation)
				})}
			})
			dst.Spec.Targets[i] = v2beta1.Target{Kafka: lo.ToPtr(v2beta1.KafkaTarget{Name: call.Name, Topics: topics})}
		}
		if call.Type == IntentTypeDatabase && len(call.DatabaseResources) > 0 {
			tables := lo.Map(call.DatabaseResources, func(resource DatabaseResource, _ int) v2beta1.SQLPrivileges {
				return v2beta1.SQLPrivileges{Table: resource.Table, DatabaseName: resource.DatabaseName, Operations: lo.Map(resource.Operations, func(operation DatabaseOperation, _ int) v2beta1.DatabaseOperation {
					return v2beta1.DatabaseOperation(operation)
				})}
			})
			dst.Spec.Targets[i] = v2beta1.Target{SQL: lo.ToPtr(v2beta1.SQLTarget{Name: call.Name, Privileges: tables})}
			continue
		}
		if call.Type == IntentTypeAWS {
			dst.Spec.Targets[i] = v2beta1.Target{AWS: lo.ToPtr(v2beta1.AWSTarget{ARN: call.Name, Actions: call.AWSActions})}
			continue
		}
		if call.Type == IntentTypeGCP {
			dst.Spec.Targets[i] = v2beta1.Target{GCP: lo.ToPtr(v2beta1.GCPTarget{Resource: call.Name, Permissions: call.GCPPermissions})}
			continue
		}
		if call.Type == IntentTypeAzure {
			dst.Spec.Targets[i] = v2beta1.Target{Azure: lo.ToPtr(v2beta1.AzureTarget{Scope: call.Name, Roles: call.AzureRoles})}
			if len(call.AzureActions) > 0 {
				dst.Spec.Targets[i].Azure.Actions = lo.Map(call.AzureActions, func(action AzureAction, _ int) v2beta1.AzureAction { return v2beta1.AzureAction(action) })
			}
			if len(call.AzureDataActions) > 0 {
				dst.Spec.Targets[i].Azure.DataActions = lo.Map(call.AzureDataActions, func(action AzureDataAction, _ int) v2beta1.AzureDataAction { return v2beta1.AzureDataAction(action) })
			}

			if call.AzureKeyVaultPolicy != nil {
				dst.Spec.Targets[i].Azure.KeyVaultPolicy = &v2beta1.AzureKeyVaultPolicy{}
				dst.Spec.Targets[i].Azure.KeyVaultPolicy.KeyPermissions = lo.Map(call.AzureKeyVaultPolicy.KeyPermissions, func(permission AzureKeyVaultKeyPermission, _ int) v2beta1.AzureKeyVaultKeyPermission {
					return v2beta1.AzureKeyVaultKeyPermission(permission)
				})
				dst.Spec.Targets[i].Azure.KeyVaultPolicy.SecretPermissions = lo.Map(call.AzureKeyVaultPolicy.SecretPermissions, func(permission AzureKeyVaultSecretPermission, _ int) v2beta1.AzureKeyVaultSecretPermission {
					return v2beta1.AzureKeyVaultSecretPermission(permission)
				})
				dst.Spec.Targets[i].Azure.KeyVaultPolicy.CertificatePermissions = lo.Map(call.AzureKeyVaultPolicy.CertificatePermissions, func(permission AzureKeyVaultCertificatePermission, _ int) v2beta1.AzureKeyVaultCertificatePermission {
					return v2beta1.AzureKeyVaultCertificatePermission(permission)
				})
				dst.Spec.Targets[i].Azure.KeyVaultPolicy.StoragePermissions = lo.Map(call.AzureKeyVaultPolicy.StoragePermissions, func(permission AzureKeyVaultStoragePermission, _ int) v2beta1.AzureKeyVaultStoragePermission {
					return v2beta1.AzureKeyVaultStoragePermission(permission)
				})
			}
		}
		if call.Type == IntentTypeInternet && call.Internet != nil {
			dst.Spec.Targets[i] = v2beta1.Target{Internet: lo.ToPtr(v2beta1.Internet{Domains: call.Internet.Domains, Ports: call.Internet.Ports, Ips: call.Internet.Ips})}
		}
	}
	return nil
}
	...
	for i, target := range src.Spec.Targets {
		...
		if target.Kubernetes != nil {
			...
			if len(target.Kubernetes.HTTP) > 0 {
				in.Spec.Calls[i].HTTPResources = lo.Map(target.Kubernetes.HTTP, func(http v2beta1.HTTPTarget, _ int) HTTPResource {
					return HTTPResource{Path: http.Path, Methods: lo.Map(http.Methods, func(method v2beta1.HTTPMethod, _ int) HTTPMethod { return HTTPMethod(method) })}
				})
		...
		if target.Service != nil {
			...
			if len(target.Service.HTTP) > 0 {
				in.Spec.Calls[i].HTTPResources = lo.Map(target.Service.HTTP, func(http v2beta1.HTTPTarget, _ int) HTTPResource {
					return HTTPResource{Path: http.Path, Methods: lo.Map(http.Methods, func(method v2beta1.HTTPMethod, _ int) HTTPMethod { return HTTPMethod(method) })}
				})
				in.Spec.Calls[i].Type = IntentTypeHTTP
			}
		...
		if target.Kafka != nil {
			in.Spec.Calls[i] = Intent{Type: IntentTypeKafka, Name: target.Kafka.Name, Topics: lo.Map(target.Kafka.Topics, func(topic v2beta1.KafkaTopic, _ int) KafkaTopic {
				return KafkaTopic{Name: topic.Name, Operations: lo.Map(topic.Operations, func(operation v2beta1.KafkaOperation, _ int) KafkaOperation { return KafkaOperation(operation) })}
			})}
			continue
		}
		if target.SQL != nil {
			in.Spec.Calls[i] = Intent{Type: IntentTypeDatabase, Name: target.SQL.Name, DatabaseResources: lo.Map(target.SQL.Privileges, func(permission v2beta1.SQLPrivileges, _ int) DatabaseResource {
				return DatabaseResource{Table: permission.Table, DatabaseName: permission.DatabaseName, Operations: lo.Map(permission.Operations, func(operation v2beta1.DatabaseOperation, _ int) DatabaseOperation {
		...
		if target.Azure != nil {
			...
			if len(target.Azure.Actions) > 0 {
				in.Spec.Calls[i].AzureActions = lo.Map(target.Azure.Actions, func(action v2beta1.AzureAction, _ int) AzureAction { return AzureAction(action) })
			}
			if len(target.Azure.DataActions) > 0 {
				in.Spec.Calls[i].AzureDataActions = lo.Map(target.Azure.DataActions, func(action v2beta1.AzureDataAction, _ int) AzureDataAction { return AzureDataAction(action) })
			}
			if target.Azure.KeyVaultPolicy == nil {
				continue
			}
			in.Spec.Calls[i].AzureKeyVaultPolicy = &AzureKeyVaultPolicy{}
			in.Spec.Calls[i].AzureKeyVaultPolicy.KeyPermissions = lo.Map(target.Azure.KeyVaultPolicy.KeyPermissions, func(permission v2beta1.AzureKeyVaultKeyPermission, _ int) AzureKeyVaultKeyPermission {
				return AzureKeyVaultKeyPermission(permission)
			})
			in.Spec.Calls[i].AzureKeyVaultPolicy.SecretPermissions = lo.Map(target.Azure.KeyVaultPolicy.SecretPermissions, func(permission v2beta1.AzureKeyVaultSecretPermission, _ int) AzureKeyVaultSecretPermission {
				return AzureKeyVaultSecretPermission(permission)
			})
			in.Spec.Calls[i].AzureKeyVaultPolicy.CertificatePermissions = lo.Map(target.Azure.KeyVaultPolicy.CertificatePermissions, func(permission v2beta1.AzureKeyVaultCertificatePermission, _ int) AzureKeyVaultCertificatePermission {
				return AzureKeyVaultCertificatePermission(permission)
			})
			in.Spec.Calls[i].AzureKeyVaultPolicy.StoragePermissions = lo.Map(target.Azure.KeyVaultPolicy.StoragePermissions, func(permission v2beta1.AzureKeyVaultStoragePermission, _ int) AzureKeyVaultStoragePermission {
