...
func (in *MySQLServerConfig) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v2.MySQLServerConfig)
	...
	if in.Spec.Credentials.SecretRef != nil {
		dst.Spec.Credentials.SecretRef = &v2.DatabaseCredentialsSecretRef{
...
func (in *MySQLServerConfig) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v2.MySQLServerConfig)
...
func (in *PostgreSQLServerConfig) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v2.PostgreSQLServerConfig)
	dst.ObjectMeta = in.ObjectMeta
	dst.Spec.Address = in.Spec.Address
	dst.Spec.Credentials.Username = in.Spec.Credentials.Username
	dst.Spec.Credentials.Password = in.Spec.Credentials.Password
...
func (in *PostgreSQLServerConfig) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v2.PostgreSQLServerConfig)
	in.ObjectMeta = src.ObjectMeta
	in.Spec.Address = src.Spec.Address
	in.Spec.Credentials.Username = src.Spec.Credentials.Username
	in.Spec.Credentials.Password = src.Spec.Credentials.Password
...
func (ksc *KafkaServerConfig) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v2.KafkaServerConfig)
...
func (ksc *KafkaServerConfig) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v2.KafkaServerConfig)
...
func (in *ProtectedService) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v2.ProtectedService)
	dst.ObjectMeta = in.ObjectMeta
...
func (in *ProtectedService) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v2.ProtectedService)
	in.ObjectMeta = src.ObjectMeta
...
func (in *ClientIntents) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v2.ClientIntents)
	dst.ObjectMeta = in.ObjectMeta
	dst.Status.UpToDate = in.Status.UpToDate
	dst.Status.ObservedGeneration = in.Status.ObservedGeneration
	dst.Status.ResolvedIPs = lo.Map(in.Status.ResolvedIPs, func(resolvedIPs ResolvedIPs, _ int) v2.ResolvedIPs {
		return v2.ResolvedIPs{DNS: resolvedIPs.DNS, IPs: slices.Clone(resolvedIPs.IPs)}
	})
	if dst.Spec == nil {
		dst.Spec = &v2.IntentsSpec{}
	}
	dst.Spec.Workload.Name = in.Spec.Service.Name
	dst.Spec.Workload.Kind = in.Spec.Service.Kind
	dst.Spec.Targets = make([]v2.Target, len(in.Spec.Calls))
	for i, call := range in.Spec.Calls {
		if call.IsTargetInCluster() && call.Type != IntentTypeKafka {
			...
			kubernetesTarget := v2.KubernetesTarget{Name: name, Kind: call.GetTargetServerKind()}
			...
			if call.Type == IntentTypeHTTP && len(call.HTTPResources) > 0 {
				httpTargets := make([]v2.HTTPTarget, len(call.HTTPResources))
				for j, http := range call.HTTPResources {
					methods := lo.Map(http.Methods, func(method HTTPMethod, _ int) v2.HTTPMethod { return v2.HTTPMethod(method) })
					httpTargets[j] = v2.HTTPTarget{Path: http.Path, Methods: methods}
				}
				kubernetesTarget.HTTP = httpTargets
			}
			dst.Spec.Targets[i] = v2.Target{Kubernetes: &kubernetesTarget}
			continue
		}
		if call.Type == IntentTypeKafka && len(call.Topics) > 0 {
			topics := lo.Map(call.Topics, func(topic KafkaTopic, _ int) v2.KafkaTopic {
				return v2.KafkaTopic{Name: topic.Name, Operations: lo.Map(topic.Operations, func(operation KafkaOperation, _ int) v2.KafkaOperation {
					return v2.KafkaOperation(operation)
				})}
			})
			dst.Spec.Targets[i] = v2.Target{Kafka: lo.ToPtr(v2.KafkaTarget{Name: call.Name, Topics: topics})}
		}
		if call.Type == IntentTypeDatabase && len(call.DatabaseResources) > 0 {
			tables := lo.Map(call.DatabaseResources, func(resource DatabaseResource, _ int) v2.SQLPrivileges {
				return v2.SQLPrivileges{Table: resource.Table, DatabaseName: resource.DatabaseName, Operations: lo.Map(resource.Operations, func(operation DatabaseOperation, _ int) v2.DatabaseOperation {
					return v2.DatabaseOperation(operation)
				})}
			})
			dst.Spec.Targets[i] = v2.Target{SQL: lo.ToPtr(v2.SQLTarget{Name: call.Name, Privileges: tables})}
			continue
		}
		if call.Type == IntentTypeAWS {
			dst.Spec.Targets[i] = v2.Target{AWS: lo.ToPtr(v2.AWSTarget{ARN: call.Name, Actions: call.AWSActions})}
			continue
		}
		if call.Type == IntentTypeGCP {
			dst.Spec.Targets[i] = v2.Target{GCP: lo.ToPtr(v2.GCPTarget{Resource: call.Name, Permissions: call.GCPPermissions})}
			continue
		}
		if call.Type == IntentTypeAzure {
			dst.Spec.Targets[i] = v2.Target{Azure: lo.ToPtr(v2.AzureTarget{Scope: call.Name, Roles: call.AzureRoles})}
			if call.AzureKeyVaultPolicy == nil {
				continue
			}
			dst.Spec.Targets[i].Azure.KeyVaultPolicy = &v2.AzureKeyVaultPolicy{}
			dst.Spec.Targets[i].Azure.KeyVaultPolicy.KeyPermissions = lo.Map(call.AzureKeyVaultPolicy.KeyPermissions, func(permission AzureKeyVaultKeyPermission, _ int) v2.AzureKeyVaultKeyPermission {
				return v2.AzureKeyVaultKeyPermission(permission)
			})
			dst.Spec.Targets[i].Azure.KeyVaultPolicy.SecretPermissions = lo.Map(call.AzureKeyVaultPolicy.SecretPermissions, func(permission AzureKeyVaultSecretPermission, _ int) v2.AzureKeyVaultSecretPermission {
				return v2.AzureKeyVaultSecretPermission(permission)
			})
			dst.Spec.Targets[i].Azure.KeyVaultPolicy.CertificatePermissions = lo.Map(call.AzureKeyVaultPolicy.CertificatePermissions, func(permission AzureKeyVaultCertificatePermission, _ int) v2.AzureKeyVaultCertificatePermission {
				return v2.AzureKeyVaultCertificatePermission(permission)
			})
			dst.Spec.Targets[i].Azure.KeyVaultPolicy.StoragePermissions = lo.Map(call.AzureKeyVaultPolicy.StoragePermissions, func(permission AzureKeyVaultStoragePermission, _ int) v2.AzureKeyVaultStoragePermission {
				return v2.AzureKeyVaultStoragePermission(permission)
			})
		}
		if call.Type == IntentTypeInternet && call.Internet != nil {
			dst.Spec.Targets[i] = v2.Target{Internet: lo.ToPtr(v2.Internet{Domains: call.Internet.Domains, Ports: call.Internet.Ports, Ips: call.Internet.Ips})}
		}
	}
	return nil
}
...
func (in *ClientIntents) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v2.ClientIntents)
	in.ObjectMeta = src.ObjectMeta
	in.Status.UpToDate = src.Status.UpToDate
	in.Status.ObservedGeneration = src.Status.ObservedGeneration
	in.Status.ResolvedIPs = lo.Map(src.Status.ResolvedIPs, func(resolvedIPs v2.ResolvedIPs, _ int) ResolvedIPs {
	...
	for i, target := range src.Spec.Targets {
		...
		if target.Kubernetes != nil {
			...
			if len(target.Kubernetes.HTTP) > 0 {
				in.Spec.Calls[i].HTTPResources = lo.Map(target.Kubernetes.HTTP, func(http v2.HTTPTarget, _ int) HTTPResource {
					return HTTPResource{Path: http.Path, Methods: lo.Map(http.Methods, func(method v2.HTTPMethod, _ int) HTTPMethod { return HTTPMethod(method) })}
				})
		...
		if target.Service != nil {
			...
			if len(target.Service.HTTP) > 0 {
				in.Spec.Calls[i].HTTPResources = lo.Map(target.Service.HTTP, func(http v2.HTTPTarget, _ int) HTTPResource {
					return HTTPResource{Path: http.Path, Methods: lo.Map(http.Methods, func(method v2.HTTPMethod, _ int) HTTPMethod { return HTTPMethod(method) })}
				})
				in.Spec.Calls[i].Type = IntentTypeHTTP
			}
		...
		if target.Kafka != nil {
			in.Spec.Calls[i] = Intent{Type: IntentTypeKafka, Name: target.Kafka.Name, Topics: lo.Map(target.Kafka.Topics, func(topic v2.KafkaTopic, _ int) KafkaTopic {
				return KafkaTopic{Name: topic.Name, Operations: lo.Map(topic.Operations, func(operation v2.KafkaOperation, _ int) KafkaOperation { return KafkaOperation(operation) })}
			})}
			continue
		}
		if target.SQL != nil {
			in.Spec.Calls[i] = Intent{Type: IntentTypeDatabase, Name: target.SQL.Name, DatabaseResources: lo.Map(target.SQL.Privileges, func(permission v2.SQLPrivileges, _ int) DatabaseResource {
				return DatabaseResource{Table: permission.Table, DatabaseName: permission.DatabaseName, Operations: lo.Map(permission.Operations, func(operation v2.DatabaseOperation, _ int) DatabaseOperation {
		...
		if target.Azure != nil {
			...
			in.Spec.Calls[i].AzureKeyVaultPolicy.KeyPermissions = lo.Map(target.Azure.KeyVaultPolicy.KeyPermissions, func(permission v2.AzureKeyVaultKeyPermission, _ int) AzureKeyVaultKeyPermission {
				return AzureKeyVaultKeyPermission(permission)
			})
			in.Spec.Calls[i].AzureKeyVaultPolicy.SecretPermissions = lo.Map(target.Azure.KeyVaultPolicy.SecretPermissions, func(permission v2.AzureKeyVaultSecretPermission, _ int) AzureKeyVaultSecretPermission {
				return AzureKeyVaultSecretPermission(permission)
			})
			in.Spec.Calls[i].AzureKeyVaultPolicy.CertificatePermissions = lo.Map(target.Azure.KeyVaultPolicy.CertificatePermissions, func(permission v2.AzureKeyVaultCertificatePermission, _ int) AzureKeyVaultCertificatePermission {
				return AzureKeyVaultCertificatePermission(permission)
			})
			in.Spec.Calls[i].AzureKeyVaultPolicy.StoragePermissions = lo.Map(target.Azure.KeyVaultPolicy.StoragePermissions, func(permission v2.AzureKeyVaultStoragePermission, _ int) AzureKeyVaultStoragePermission {
