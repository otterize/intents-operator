...
func (ksc *KafkaServerConfig) ConvertTo(dstRaw conversion.Hub) error {
	...
	dst.Spec = v2.KafkaServerConfigSpec{}
	dst.Spec.Addr = ksc.Spec.Addr
	dst.Spec.Service = v2.Workload{Name: ksc.Spec.Service.Name}
	...
	for _, topic := range ksc.Spec.Topics {
		dst.Spec.Topics = append(dst.Spec.Topics, v2.TopicConfig{
			Topic:                  topic.Topic,
			Pattern:                v2.ResourcePatternType(topic.Pattern), // this casting is fine as v1alpha2 == v1alpha3
