...
func (r *NetworkPolicyHandler) HandleAllServices(ctx context.Context, namespace string) error {
	...
	err = r.HandleAllPods(ctx, namespace)
	if err != nil {
		return errors.Wrap(err)
	}
...
func (r *NetworkPolicyHandler) reducedNetworkPoliciesInNamespace(ctx context.Context, pods []PotentiallyScrapeMetricPod) (NetworkPolicyByName, error) {
	...
	for _, pod := range pods {
		if !r.shouldScrape(pod.scrapeResourceMeta) {
			continue
		}

		netpol, err := r.handlePod(ctx, &pod)
		if err != nil {
...
func (r *NetworkPolicyHandler) handlePod(ctx context.Context, pod *PotentiallyScrapeMetricPod) (v1.NetworkPolicy, error) {
	...
	netpolSlice, err := r.getAllPossibleNetworkPolicies(ctx, pod.pod, serviceId)
	if err != nil {
		return v1.NetworkPolicy{}, errors.Wrap(err)
	}
...
func (r *NetworkPolicyHandler) shouldScrape(resource *metav1.ObjectMeta) bool {
...
func (r *NetworkPolicyHandler) getMetricsPort(resource *metav1.ObjectMeta) (int32, error) {
	...
	portValue := resource.Annotations["prometheus.io/port"]
	if portValue == "" {
		// TODO: should I return something else here?
		return 0, nil
	}
