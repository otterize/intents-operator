...
func (s *NetworkPolicyReconcilerTestSuite) testCreateNetworkPolicy(
	...
	intentsSpec := &otterizev2.IntentsSpec{
		...
		Targets: []otterizev2.Target{
			{
				Kubernetes: &otterizev2.KubernetesTarget{Name: fmt.Sprintf("test-server.%s", serverNamespace)},
			},
		},
	}
...
func (s *NetworkPolicyReconcilerTestSuite) TestUpdateNetworkPolicy() {
	...
	s.expectGetAllEffectivePolicies([]otterizev2.ClientIntents{{ObjectMeta: metav1.ObjectMeta{Namespace: req.Namespace, Name: req.Name}, Spec: intentsSpec}})
			...
			From: []v1.NetworkPolicyPeer{
				{
					PodSelector: &metav1.LabelSelector{
						MatchLabels: map[string]string{
							fmt.Sprintf(
								otterizev2.OtterizeAccessLabelKey, formattedTargetServer): "true",
						},
					},
					NamespaceSelector: &metav1.LabelSelector{
						MatchLabels: map[string]string{
							otterizev2.KubernetesStandardNamespaceNameLabelKey: namespace,
						},
					},
				},
			},
...
func (s *NetworkPolicyReconcilerTestSuite) testServerNotProtected(clientIntentsName string, serverName string, serverNamespace string, serviceName string) {
	...
	intentsSpec := &otterizev2.IntentsSpec{
		...
		Targets: []otterizev2.Target{
			{
				Kubernetes: &otterizev2.KubernetesTarget{Name: serverFullName},
			},
		},
	}
...
func (s *NetworkPolicyReconcilerTestSuite) testEnforcementDisabled() {
	...
	s.ignoreRemoveOrphan()
	s.Client.EXPECT().List(gomock.Any(), gomock.Eq(&otterizev2.ProtectedServiceList{}), gomock.Any()).AnyTimes().DoAndReturn(
		func(ctx context.Context, list *otterizev2.ProtectedServiceList, opts ...client.ListOption) error {
			return nil
		})
...
func (s *NetworkPolicyReconcilerTestSuite) TestPolicyNotDeletedForTwoClientsWithSameServer() {
	...
	otherClientIntentsInTheNamespace := otterizev2.ClientIntents{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "other-client-intents",
			Namespace: testNamespace,
		},
		Spec: &otterizev2.IntentsSpec{
			Workload: otterizev2.Workload{Name: "other-client"},
			Targets: []otterizev2.Target{
				{
					Kubernetes: &otterizev2.KubernetesTarget{Name: serverName},
				},
	...
	s.expectGetAllEffectivePolicies([]otterizev2.ClientIntents{clientIntentsObj, otherClientIntentsInTheNamespace})
...
func (s *NetworkPolicyReconcilerTestSuite) TestTwoClientsFromDifferentNamespacesWithSameServer() {
	...
	otherClientIntentsDifferentNamespace := otterizev2.ClientIntents{
		ObjectMeta: metav1.ObjectMeta{
	...
	s.expectGetAllEffectivePolicies([]otterizev2.ClientIntents{clientIntentsObj, otherClientIntentsDifferentNamespace})
