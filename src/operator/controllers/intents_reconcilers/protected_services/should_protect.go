...
func IsServerEnforcementEnabledDueToProtectionOrDefaultState(ctx context.Context, kube client.Client, serverServiceId serviceidentity.ServiceIdentity, enforcementDefaultState bool, activeNamespaces *goset.Set[string]) (bool, error) {
	...
	err := kube.List(ctx, &protectedServicesResources,
		client.MatchingFields{otterizev2.OtterizeProtectedServiceNameIndexField: serverServiceId.Name},
...
func InitProtectedServiceIndexField(mgr ctrl.Manager) error {
	err := mgr.GetCache().IndexField(
		context.Background(),
		&otterizev2.ProtectedService{},
		otterizev2.OtterizeProtectedServiceNameIndexField,
		func(object client.Object) []string {
			protectedService := object.(*otterizev2.ProtectedService)
