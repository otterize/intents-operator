...
func (r *CustomResourceDefinitionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if bytes.Equal(crd.Spec.Conversion.Webhook.ClientConfig.CABundle, r.certPem) && (crd.Spec.Conversion.Webhook.ClientConfig.Service == nil || crd.Spec.Conversion.Webhook.ClientConfig.Service.Namespace == r.namespace) {
	...
	resourceCopy.Spec.Conversion.Webhook.ClientConfig.URL = lo.ToPtr(fmt.Sprintf("https://host.minikube.internal:9443/%s",
		lo.FromPtr(resourceCopy.Spec.Conversion.Webhook.ClientConfig.Service.Path)))
