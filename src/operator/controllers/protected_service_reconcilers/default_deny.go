...
func (r *DefaultDenyReconciler) blockAccessToServices(ctx context.Context, protectedServices otterizev2.ProtectedServiceList, namespace string) error {
	...
	for _, existingPolicy := range networkPolicies.Items {
		existingPolicyServerName := existingPolicy.Labels[otterizev2.OtterizeNetworkPolicy]
...
func (r *DefaultDenyReconciler) buildNetworkPolicyObjectForIntent(ctx context.Context, serviceId serviceidentity.ServiceIdentity) (v1.NetworkPolicy, bool, error) {
	...
	podSelectorLabels, ok, err := otterizev2.ServiceIdentityToLabelsForWorkloadSelection(ctx, r.Client, serviceId)
	if err != nil {
	...
	netpol := v1.NetworkPolicy{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				otterizev2.OtterizeNetworkPolicyServiceDefaultDeny: "true",
				otterizev2.OtterizeNetworkPolicy:                   serviceId.GetFormattedOtterizeIdentityWithoutKind(),
			},
		},
