	...
	toBeRemovedPolicy := &v1.NetworkPolicy{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				otterizev2.OtterizeNetworkPolicy: serviceName,
			},
		},
		Spec: v1.NetworkPolicySpec{
			...
			PodSelector: metav1.LabelSelector{
				MatchLabels: map[string]string{
					otterizev2.OtterizeServiceLabelKey: serviceName,
				},
			},
		},
	}
...
func (s *NetworkPolicyHandlerTestSuite) TestNetworkPolicyHandler_HandleBeforeAccessPolicyRemoval_onlyOneRelatedPolicy_shouldDelete() {
	...
	externalPolicy := &v1.NetworkPolicy{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: serviceNamespace,
			Labels: map[string]string{
				otterizev2.OtterizeNetworkPolicyExternalTraffic: serviceName,
			},
		},
		Spec: v1.NetworkPolicySpec{
			PolicyTypes: []v1.PolicyType{v1.PolicyTypeIngress},
	...
	firstList := s.Client.EXPECT().List(
		gomock.Any(), gomock.Eq(&v1.NetworkPolicyList{}), client.MatchingLabels{otterizev2.OtterizeNetworkPolicy: serviceName}, &client.ListOptions{Namespace: serviceNamespace},
	).DoAndReturn(
	...
	secondList := s.Client.EXPECT().List(
		...
		client.MatchingLabels{otterizev2.OtterizeNetworkPolicyExternalTraffic: serviceName},
