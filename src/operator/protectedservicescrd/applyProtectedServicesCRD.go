...
func EnsureProtectedServicesCRD(ctx context.Context, client client.Client) error {
	...
	if err != nil {
		return errors.Errorf("failed to unmarshal protected services CRD: %w", err)
	}
	crd := apiextensionsv1.CustomResourceDefinition{}
	err = client.Get(ctx, types.NamespacedName{Name: crdToCreate.Name}, &crd)
	if err != nil && !k8serrors.IsNotFound(err) {
		return errors.Errorf("could not get protected services CRD: %w", err)
	}

	if k8serrors.IsNotFound(err) {
		err := client.Create(ctx, &crdToCreate)
		if err != nil {
			return errors.Errorf("could not create protected services CRD: %w", err)
		}
		return nil
	}
