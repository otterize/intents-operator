...
func (s *ValidationWebhookTestSuite) TestOnlyOneTargetTypeAllowed() {
	_, err := s.AddIntentsv2("intents", "someclient", []otterizev2.Target{
		{
			Kafka: &otterizev2.KafkaTarget{
				Topics: []otterizev2.KafkaTopic{{
					Name:       "sometopic",
					Operations: []otterizev2.KafkaOperation{otterizev2.KafkaOperationConsume},
				}},
			},
	...
	_, err = s.AddIntentsv2("intents", "someclient", []otterizev2.Target{
		{
			...
			Kubernetes: &otterizev2.KubernetesTarget{Name: "omriservice", Kind: "Deployment"},
		},
	})
	s.Require().ErrorContains(err, "each target must have exactly one field set")
	...
	_, err = s.AddIntentsv2("intents", "someclient", []otterizev2.Target{
		{
			...
			SQL: &otterizev2.SQLTarget{
				Privileges: []otterizev2.SQLPrivileges{{
					...
					Table:        "sometable",
					Operations:   []otterizev2.DatabaseOperation{otterizev2.DatabaseOperationSelect},
				}},
			},
		},
	})
...
func (s *ValidationWebhookTestSuite) TestNoDuplicateClientsAllowed() {
	...
	_, err = s.AddIntentsv2("intents2", "someclient", []otterizev2.Target{})
...
func (s *ValidationWebhookTestSuite) TestNameRequiredForEveryTypeExceptInternet() {
	...
	_, err := s.AddIntentsv2("kafka-intents", "kafka-client", []otterizev2.Target{
		{
			Kafka: &otterizev2.KafkaTarget{
				Topics: []otterizev2.KafkaTopic{{
					Name:       "sometopic",
	...
	_, err = s.AddIntentsv2("http-intents", "http-client", []otterizev2.Target{
		{
			Kubernetes: &otterizev2.KubernetesTarget{
				HTTP: []otterizev2.HTTPTarget{{
					Path:    "/somepath",
					Methods: []otterizev2.HTTPMethod{otterizev2.HTTPMethodGet},
				}},
			},
		},
	})
	...
	_, err = s.AddIntentsv2("database-intents", "database-client", []otterizev2.Target{
		{
			SQL: &otterizev2.SQLTarget{
				Privileges: []otterizev2.SQLPrivileges{{
					DatabaseName: "sadfsdf",
	...
	_, err = s.AddIntentsv2("aws-intents", "aws-client", []otterizev2.Target{
		{
			AWS: &otterizev2.AWSTarget{
				Actions: []string{"s3:GetObject"},
			},
	...
	_, err = s.AddIntentsv2("azure-intents", "aws-client", []otterizev2.Target{
		{
			Azure: &otterizev2.AzureTarget{
				Roles: []string{"Contributor"},
			},
	...
	_, err = s.AddIntentsv2("gcp-intents", "aws-client", []otterizev2.Target{
		{
			GCP: &otterizev2.GCPTarget{
	...
	_, err = s.AddIntentsv2("internet-intents", "internet-client", []otterizev2.Target{
		{
			Internet: &otterizev2.Internet{
				Ips:   []string{"1.1.1.1"},
				Ports: []int{80},
...
// Validate the conversion between v1alpha3 and v2 works properly
func (s *ConversionWebhookTestSuite) TestConversionWebhookRegularIntents() {
	...
	// Convert the v1alpha3 object to a v2 object
	v2Obj := &otterizev2.ClientIntents{}
	err := v1alpha3Intents.ConvertTo(v2Obj)
	s.Require().NoError(err)

	// Convert the v2 object back to a v1alpha3 object
	anotherV1alpha3Intents := &otterizev1alpha3.ClientIntents{}
	err = anotherV1alpha3Intents.ConvertFrom(v2Obj)
	s.Require().NoError(err)
...
// Validate the conversion between v1alpha3 and v2 works properly - kafka intents
...
// Validate the conversion between v1alpha3 and v2 works properly - http intents
...
// Validate the conversion between v1alpha3 and v2 works properly - database intents
...
// Validate the conversion between v1alpha3 and v2 works properly - aws intents
...
// Validate the conversion between v1alpha3 and v2 works properly - azure intents
...
// Validate the conversion between v1alpha3 and v2 works properly - gcp intents
...
// Validate the conversion between v1alpha3 and v2 works properly - internet intents
