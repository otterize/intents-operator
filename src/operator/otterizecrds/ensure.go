...
func ensureCRD(ctx context.Context, k8sClient client.Client, operatorNamespace string, crdContent []byte, certPem []byte) error {
	...
	crdToCreate.Spec.Conversion.Webhook.ClientConfig.URL = lo.ToPtr(fmt.Sprintf("https://host.minikube.internal:9443/%s",
		lo.FromPtr(crdToCreate.Spec.Conversion.Webhook.ClientConfig.Service.Path)))
