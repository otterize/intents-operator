...
func sanitizeFormatInput(formatInput any) (string, error) {
	if dbOperations, ok := formatInput.([]otterizev2.DatabaseOperation); ok {
		asStrings := lo.Map(dbOperations, func(op otterizev2.DatabaseOperation, _ int) string { return string(op) })
