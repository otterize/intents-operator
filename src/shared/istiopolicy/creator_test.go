...
func (s *CreatorTestSuite) TestCreatePolicyWithMultipleIntents() {
	clientName := "test-client"
	serverName1 := "test-server-1"
	serverName2 := "test-server-2"
	policyName1 := "authorization-policy-to-test-server-1-from-test-client.test-namespace"
	policyName2 := "authorization-policy-to-test-server-2-from-test-client.test-namespace"
	clientIntentsNamespace := "test-namespace"
	...
	principal := generatePrincipal(clientIntentsNamespace, clientServiceAccountName)
	newPolicy1 := &v1beta1.AuthorizationPolicy{
		ObjectMeta: v1.ObjectMeta{
			Name:      policyName1,
			Namespace: clientIntentsNamespace,
			Labels: map[string]string{
				v1alpha2.OtterizeServerLabelKey:           "test-server-1-test-namespace-1628ef",
				v1alpha2.OtterizeIstioClientAnnotationKey: "test-client-test-namespace-537e87",
			},
		},
		Spec: v1beta12.AuthorizationPolicy{
	...
	newPolicy2 := &v1beta1.AuthorizationPolicy{
		ObjectMeta: v1.ObjectMeta{
			Name:      policyName2,
			Namespace: clientIntentsNamespace,
			Labels: map[string]string{
				v1alpha2.OtterizeServerLabelKey:           "test-server-2-test-namespace-e4423b",
				v1alpha2.OtterizeIstioClientAnnotationKey: "test-client-test-namespace-537e87",
			},
		},
		Spec: v1beta12.AuthorizationPolicy{
	...
	s.mockClient.EXPECT().List(gomock.Any(), gomock.Any(), gomock.AssignableToTypeOf(client.MatchingLabels{})).Return(nil)
	s.mockClient.EXPECT().Create(gomock.Any(), newPolicy1).Return(nil)
	s.mockClient.EXPECT().Create(gomock.Any(), newPolicy2).Return(nil)
